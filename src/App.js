...
const Modal = (props) => {
    ...
    return (
        <Transition
            in={props.show}
            timeout={duration}
            onEnter={() => props.setShowBtnTrigger(false)}
            onExited={() => props.setShowBtnTrigger(true)}
        >
            {state => (
...
function App() {
    ...
    const [showBtnTrigger, setShowBtnTrigger] = useState(true);

    console.log('rerender');
    console.log();
    return (
        <Container>
            <Modal show={showModal} onClose={setShowModal} setShowBtnTrigger={setShowBtnTrigger}/>
            {showBtnTrigger ?
                <button
                    type="button"
